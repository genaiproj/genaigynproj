...
def load_encounters(raw_xlsx: bytes) -> pd.DataFrame:
    ...
    pid_col = find_col(["patientid", "patient_id", "PatientID", "PATIENT_ID", "MRN","patient firstname","patient lastname"])
    ...
    return enc[["patient_id", "first_name", "last_name", "encounter_date", "location", "cpt_code", "ICD_10_1","ICD_10_2","ICD_10_3","ICD_10_4"]].copy()
...
def write_report(rows: List[Dict], evidence_df: pd.DataFrame, meta: Dict, out_path: str) -> None:
    ...
    if last_row >= header_row + 1:
        tbl = Table(displayName="FQHC", ref=f"A{header_row}:I{last_row}")
...
def main() -> None:
    ...
    for _, row in hidden_df.iterrows():
            rows.append({
                ...
                "ICD_10_1": row["ICD_10_1"],
                "ICD_10_2": row["ICD_10_2"],
                "ICD_10_3": row["ICD_10_3"],
                "ICD_10_4": row["ICD_10_4"],
            })

    #rows.sort(key=lambda r: (r["patient_id"], r["encounter_date"]))
...
''''    
    ...
    diag_mask = (
        enc_df[diag_cols]
        .fillna("")
    .   #apply(lambda col: col.str.strip().str.upper().str.startswith("O"))
        .apply(lambda col: col.str.strip().str.upper().str.startswith("O"))
        .any(axis=1)
    )
    ...
    mask = (
        enc_df["location"].isin(HOSPITAL_LOCATIONS)
        & enc_df["cpt_code"].astype(str).str.startswith("99")
        & diag_mask
    )

    # 3. Apply the filter
    enc_df_hd_prg = enc_df[mask].copy()
    #enc_df_hd_prg = enc_df[enc_df["location"].isin(HOSPITAL_LOCATIONS))].copy() 

    hidden_preg = sorted(set(enc_df_hd_prg["patient_id"]) - set(latest_df["patient_id"]))
    hidden_preg_set = set(hidden_preg)
    hidden_count = len(hidden_preg)

    #hidden_all = hidden_all.dropna(subset=["encounter_date"]).sort_values(["patient_id", "encounter_date", "cpt_code"])
''''       
    ...
    hidden_df = enc_df_hd_prg[enc_df_hd_prg["patient_id"].isin(hidden_preg_set)]    

    rows: List[Dict] = []
    for pid in hidden_preg:
        for _, row in hidden_df.iterrows():    
            rows.append({
                "patient_id": row["patient_id"],
                "last_name": row["last_name"], 
                "first_name": row["first_name"], 
                "date_of_birth": row["date_of_birth"].strftime("%Y-%m-%d %H:%M:%S UTC"),
                "encounter_date": row["encounter_date"].strftime("%Y-%m-%d %H:%M:%S UTC"), 
                "location": row["location"], 
                "cpt_code": row["cpt_code"], 
                "ICD_10_1": row["ICD_10_1"],
                "ICD_10_2": row["ICD_10_2"],
    ...
    rows.sort(key=lambda r: (str(r.get("patient_id", ""))))
