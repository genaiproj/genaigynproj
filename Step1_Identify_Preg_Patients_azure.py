...
def classify_diabetes(dx_codes: set[str], dx_desc_text: str) -> tuple[str | None, str]:
    ...
    personal_hist_pattern = re.compile(r"\b(personal\s+)?(history|hx)\b[\s:,-]*\bgestational\s+(diabetes|dm)\b",re.IGNORECASE)
    if any(code.startswith("Z86.32") for code in dx_codes):
        return "Personal history of gestational diabetes", "ICD confirmed"
    elif personal_hist_pattern.search(t):
        return "Personal history of gestational diabetes", "Text inferred"    
...
class PregnantPatientsIdentifier:
    ...
    def identify_complications(self, patient_group: pd.DataFrame):
        ...
        #Added on 2026-03-03 to catch words after diabetes
        screening_diabetes_regex = r"\b(encounter\s+for\s+)?screen(?:ing)?\s+for\s+diabetes\b.*"
